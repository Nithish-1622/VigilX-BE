...
class JSONConnector(BaseConnector):
    ...
    def __init__(self, connection_string: str, **kwargs):
        ...
        if not os.path.exists(self.connection_string):
            raise ValueError(f"JSON file not found: {self.connection_string}")
    ...
    async def discover_schema(self) -> Dict[str, Any]:
        with open(self.connection_string, 'r', encoding='utf-8') as f:
            ...
            if isinstance(data, dict):
                ...
                records = data[:100] if isinstance(data, list) else []
    ...
    async def stream(self, batch_size: int = 1000) -> AsyncGenerator[List[Record], None]:
        ...
        # Using ijson for streaming large JSON files without loading into memory
        batch = []
        with open(self.connection_string, 'rb') as f:
            for item in ijson.items(f, 'item'):
                batch.append(await self.normalize(item))
    ...
    async def normalize(self, raw_data: Dict[str, Any]) -> Record:
        ...
        record_id = normalized.pop('id', None) or normalized.pop('_id', None)
        return Record(
